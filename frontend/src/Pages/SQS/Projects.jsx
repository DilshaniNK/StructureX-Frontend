...
import DailyUpdatesView from '../../Components/SQS/DailyUpdatesView'
import SiteVisitList from '../../Components/SQS/SiteVisitList'
import PaymentPlanView from '../../Components/SQS/PaymentPlanView'
...
function Projects() {
  ...
  // Fetch projects from API - SQS gets all projects
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      try {
        // SQS gets all projects
        const url = `http://localhost:8086/api/v1/qs/projects/all`
  ...
  // Fetch BOQ data when a project is selected and boq section is active
  useEffect(() => {
  ...
  // Fetch Payment data when a project is selected and overview section is active
  useEffect(() => {
  ...
  const getFilteredProjects = () => {
    ...
    // For SQS, filter by showMyProjects toggle (filter by qs_id matching SQS employee ID)
    const currentEmployeeId = employeeId || SQS_EMPLOYEE_ID
    if (showMyProjects) {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="grid grid-cols-12 gap-2 text-sm font-medium text-gray-700">
                        ...
                        <div className="col-span-3">Parent Task</div>
                          ...
                          <div className="col-span-3">
                            {bulkSubtaskParent ? (
                              <div className="px-3 py-2 bg-gray-100 border border-gray-300 rounded-md text-sm text-gray-700">
                                Parent: {findTaskById(wbsData, bulkSubtaskParent)?.name || 'Selected Task'}
                              </div>
                            ) : (
                              <select
                                value={task.parentId || ''}
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div className="text-sm text-blue-800">
                          ...
                          <ul className="list-disc list-inside space-y-1">
                            <li>Task name is required for each task</li>
                            <li>Select "None (Root)" for parent to create top-level tasks</li>
                            <li>Choose an existing task as parent to create subtasks</li>
                            <li>Check milestone to mark important tasks</li>
                            <li>You can add as many tasks as needed</li>
                          </ul>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t">
                      <div className="text-sm text-gray-600">
                        {bulkTasks.filter(t => t.name.trim()).length} of {bulkTasks.length} task(s) with names
                      </div>
                      <div className="flex space-x-3">
                        ...
                        <button
                          ...
                          disabled={bulkTasks.filter(t => t.name.trim()).length === 0}
                          className={`px-4 py-2 rounded-md transition-colors ${
                            bulkTasks.filter(t => t.name.trim()).length === 0
                              ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                              : 'bg-green-600 text-white hover:bg-green-700'
                          }`}
                        >
                          Create {bulkTasks.filter(t => t.name.trim()).length} Task(s)
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            ...
            {loadingBoq ? (
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium text-blue-900">{boqData.boq.boqId}</p>
                        </div>
                        <div>
                          <p className="text-blue-600">Date</p>
                          <p className="font-medium text-blue-900">{boqData.boq.date}</p>
                        </div>
                        <div>
                          <p className="text-blue-600">QS Officer</p>
                          <p className="font-medium text-blue-900">{boqData.boq.qsId}</p>
                        </div>
                        <div>
                          <p className="text-blue-600">Total Items</p>
                          <p className="font-medium text-blue-900">{boqData.items.length}</p>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-3xl font-bold text-blue-900">
                        Rs {boqData.items.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}
                      </p>
                      {/* Only show Edit button if the project's qs_id matches the current SQS employee ID */}
                      {selectedProject?.qs_id === (employeeId || SQS_EMPLOYEE_ID) ? (
                        <button 
                          onClick={() => navigate('/sqs/boq', { state: { editBoqId: boqData.boq.boqId, projectId: selectedProject.project_id } })}
                        ...
                        <div className="mt-3 text-xs text-gray-600 bg-gray-100 px-3 py-2 rounded-md">
                          <p>BOQ managed by QS: {boqData.boq.qsId}</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
                ...
                {/* Only show Add BOQ button if the project's qs_id matches the current SQS employee ID */}
                {selectedProject?.qs_id === (employeeId || SQS_EMPLOYEE_ID) ? (
                  <button 
                    onClick={() => navigate('/sqs/boq', { state: { createForProject: selectedProject.project_id } })}
                ...
                <div className="md:col-span-2 border-t pt-4 mt-2">
                  <div className="flex flex-wrap gap-3">
                    ...
                    >
                      ...
                      <span>{showClientInfo ? 'Hide' : 'View'} Client Information</span>
                      <svg className={`w-4 h-4 transform transition-transform ${showClientInfo ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                      ...
                      <span>{showProjectTeam ? 'Hide' : 'View'} Project Team</span>
                      <svg className={`w-4 h-4 transform transition-transform ${showProjectTeam ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                      </svg>
                    </button>
                  </div>
