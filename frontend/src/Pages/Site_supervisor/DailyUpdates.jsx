...
const DailyUpdates = () => {
  ...
  const [dailyupdates, setDailyRecords] = useState([]);
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch(`${API_BASE}/projects/${employeeId}`);
  ...
  const fetchDailyRecords = async () => {
    try {
      const res = await fetch(`${API_BASE}/daily_updates/${employeeId}`);
      if (!res.ok) throw new Error('Failed to fetch daily updated records');
      const data = await res.json();
  ...
  const handleProjectChange = (e) => {
    ...
    setFormData({
      ...formData,
      projectId,
      employee_id: project ? project.ssId : '',
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.projectId || !formData.note || !formData.date) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/daily_updates`, {
        method,
      ...
      setSuccess(editing ? 'Daily record updated successfully' : 'Daily update uploaded successfully');
      setFormData({
        projectId: '',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/daily_updates/${id}`, { method: 'DELETE' });
      if (!res.ok) {
        const errData = await res.json();
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-slate-800">Daily Updates </h1>
              <p className="text-slate-600 mt-1">Inform and manage sites by providing daily updates</p>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-6 border-b border-slate-200">
                ...
                <p className="text-sm text-slate-600 mt-1">{dailyupdates.length} total records</p>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {dailyupdates.length > 0 ? (
                      dailyupdates.map(record => (
                        <tr key={record.updateId} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm text-slate-800 font-semibold"> {record.note}</td>
                          ...
                          <td className="px-6 py-4 text-sm">
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => handleDelete(record.updateId)}
